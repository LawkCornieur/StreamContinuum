...
if __name__ == '__main__':
    ...
    if not monitor.waitForAbort(3):
        ...
        if ADDON.getSettingBool('auto_start'):
            xbmc.log("StreamContinuum: Waiting for GUI window stabilization before auto-start.", xbmc.LOGINFO)
            gui_timeout = 50
            while gui_timeout > 0 and not (xbmc.getCondVisibility('Window.IsActive(home)') or xbmc.getCondVisibility('Window.IsVisible(home)')):
                xbmc.sleep(100)
                gui_timeout -= 1
            xbmc.sleep(1000)
